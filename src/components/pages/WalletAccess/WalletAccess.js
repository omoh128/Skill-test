...
import { WalletConnectProvider } from '@walletconnect/web3-provider'; // Add this line
...
function Launch(props) {
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem('userInfo')) {
      openNotification(t('Access failed.'), t('Please log in.'), false, () =>
  ...
  async function accessWalletWithPrivateKey() {
    ...
    const response = await axios
      ...
      .then((response) => {
        ...
        } else openNotification(t('Access failed.'), response.data.message, false);
      })
      .catch((errorInfo) => {
